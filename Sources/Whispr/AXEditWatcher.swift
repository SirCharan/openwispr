...
final class AXEditWatcher {
    ...
    /// paired by the spelling band (JW 0.70–0.95) or case-only difference.
    static func pairs(transcript: String, before: String, after: String) -> [(from: String, to: String)] {
        ...
        for from in gone {
            for to in appeared where out.count < 3 {
                if from.lowercased() == to.lowercased(), from != to {
                    out.append((from, to)); break
                }
                let jw = DictionaryStore.jaroWinkler(from.lowercased(), to.lowercased())
                if jw >= 0.70 && jw <= 0.95 { out.append((from, to)); break }
            }
        }
        return out
    }
